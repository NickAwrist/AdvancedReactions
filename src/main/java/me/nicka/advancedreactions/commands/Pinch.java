...
public class Pinch {
    public Pinch(){
        ...
        new CommandBase("pinch", 1, 1, true){
            @Override
            public boolean onCommand(CommandSender sender, String[] arguments){
                ...
                Particle p = Particle.valueOf(config.getString("Messages.Pinch.Particle"));
                Sound s = Sound.valueOf(config.getString("Messages.Pinch.Sound"));

                if(config.getString("Messages.Pinch.Sound").equals(""))
                    s = Sound.ENTITY_PUFFER_FISH_AMBIENT;
                if(config.getString("Messages.Pinch.Particle").equals(""))
