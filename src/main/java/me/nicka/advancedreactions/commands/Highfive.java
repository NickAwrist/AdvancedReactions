    ...
    public Highfive(){
        new CommandBase("highfive", 1, 1, true){
            ...
            public boolean onCommand(CommandSender sender, String[] arguments){
                ...
                Msg.send(p_receiver, config.getString("Messages.Highfive.MessageToReceiver").replace("%SENDER%", p_sender.getName()));
                Msg.send(p_sender, config.getString("Messages.Highfive.MessageToSender").replace("%RECEIVER%", p_receiver.getName()));
